...
resource "null_resource" "docker-compose" {
  triggers = {
    dockercompose_sha1 = "${sha1(file("docker-compose.yml"))}"
    dockerfile_sha1 = "${sha1(file("Dockerfile"))}"
    install_sha1 = "${sha1(file("spinnaker-install.sh"))}"
    tfvars_sha1 = "${sha1(file("terraform.tfvars"))}"
  }
